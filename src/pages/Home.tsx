        ...
        <div className="rounded-xl border border-ci-border bg-ci-panel p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            {notifications.length > 0 && (
              <Link to="/modules/notifications" className="text-[11px] text-ci-accent">
                View all →
              </Link>
            )}
          </div>
          <ul className="space-y-2.5">
            {notifications.slice(0, 4).map((n) => (
              <li key={n.id} className="flex items-start justify-between gap-3 text-sm">
                <span>{n.text}</span>
                <Link to={n.moduleHref} className="shrink-0 text-[11px] text-ci-accent">
                  {n.moduleName}
                </Link>
              </li>
            ))}
            {notifications.length === 0 && <li className="text-sm text-ci-muted">Nothing needs your attention.</li>}
          </ul>
        </div>
        <div className="rounded-xl border border-ci-border bg-ci-panel p-4">
          <div className="flex items-center justify-between mb-3">
